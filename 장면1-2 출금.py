...
def secondButtonTapped():
    label2.config(text = "↪출금할 금액을 입력하세요")
...
def thirdButtonTapped():
    ...
    label2.config(text = "↪출금이 완료되었습니다")
    label2.place(x = 100, y = 500)
    entry1.destroy()
    button1.config(text = "계좌잔액 확인하기")
    button1.place(x = 160, y = 570)
...
def fourthButtonTapped():
    ...
    label2.config(text = "↪출금 후 당신의 계좌잔액은\n"+ str(label6.cget("text")) + "(만)원 입니다")
...
#레이블
label1 = Label(window, text="출금" , font=("Arial", 24, "bold"), fg="white", bg="navy")
label1.place(x =210 , y = 55)
label2 = Label(window, text="↪당신의 계좌번호를 입력하세요",font=("Arial", 20, "bold"), fg="white", bg="navy")
...
label7 = Label(window, text= "출금금액", font=("Arial", 18, "bold"), fg="white", bg="blue")
